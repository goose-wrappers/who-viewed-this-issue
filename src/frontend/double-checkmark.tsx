import {Image} from '@forge/react';
import React from 'react';

export const DoubleCheckmark: React.FC = () => {
	const PNG64 = "data:image/png;base64,";
	const ICON64 = "";
	const PADDING100 = "iVBORw0KGgoAAAANSUhEUgAAAGQAAAABCAQAAACC0sM2AAAADElEQVR42mNkGCYAAAGSAAIVQ4IOAAAAAElFTkSuQmCC";

	return (
		<>
			<Image alt="" src={PNG64 + ICON64}/>
			{/* a padding is added because all images have margin:auto */}
			<Image alt="" src={PNG64 + PADDING100}/>
		</>
	);
}
